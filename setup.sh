...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/R2GANTENG/Register/main/reg | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/R2GANTENG/Register/main/reg | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/R2GANTENG/Register/main/reg | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/R2GANTENG/VIPR2MTUNNEL/main/"
...
echo ""
if [[ $host == "1" ]]; then
echo -e "   \e[1;32mSILAHKAN MASUKAN DOMAIN $NC"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/R2GANTENG/Register/main/reg | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/R2GANTENG/Register/main/reg | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────</code>
<b>⚡AUTOSCRIPT R2M PREMIUM⚡</b>
<code>────────────────────</code>
...
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ🐳","url":"https://t.me/supra_store31"},{"text":"grub🐬","url":"https://t.me/nusantaraVpn"}]]}'
...
echo -e "${YELLOW} SCRIPT SUDAH DI INSTALL"
echo ""
read -p "$( echo -e "Press ${RED}[ ${NC}${RED}Enter${NC} ${RED}]${NC} For Reboot") "
reboot
